...
def main():
    ...
    ap.add_argument("--id-field", default="listingId", help="Name of a synthetic @ONEUP unique-id field to add to every top-level document (use --no-id-field to disable)")
    ap.add_argument("--no-id-field", action="store_true", help="Don't add the synthetic unique-id field")
    args = ap.parse_args()
    ...
    if not args.no_id_field:
        level_record_count = len([r for r in records if isinstance(r, dict)])
        write_csv_gz(
            args.out_dir,
            sanitize_name(args.id_field, set()) + ".csv.gz",
            [args.id_field, "probability"],
            [["@ONEUP", level_record_count]],
            report,
        )
        print("Added synthetic unique-id field '{}' (@ONEUP).".format(args.id_field))
